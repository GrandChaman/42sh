...
#    Updated: 2018/04/22 15:30:12 by fle-roy          ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
DEP_DIR = dep
INCLUDE = includes
VPATH = $(shell find srcs -type d | tr '\n' ':')
	# srcs:srcs/cli:srcs/term:srcs/builtins \
...
SRC =  $(shell find srcs -regex '.*\.c' -exec basename {} \; | tr '\n' ' ')
# cli.c cli_init.c \
...
CFLAG = -g3 -Wall -Wextra -fsanitize=address -I $(LIBFT_INCLUDE) -I $(INCLUDE)
CC = cc
LFLAG = -ltermcap
BIN = bin
OBJ = $(SRC:%.c=$(OBJ_DIR)/%.o)
